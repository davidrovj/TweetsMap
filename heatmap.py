...
def get_osm_background(config, padding):
    ...
    #if the layer option is on we only draw the points
    if config.layer:
        from PIL import ImageDraw
        transparent_area = (0,0,image.size[0], image.size[1])
...
class Configuration(object):
    ...
    def _make_optparser(self):
        ...
        optparser.add_option('', '--layer', action='store_true', help='only plot points')
...
def main():
    ...
    logging.debug('hsva_min: {}/n hsva_max: {}'.format(options.hsva_min, options.hsva_max))
